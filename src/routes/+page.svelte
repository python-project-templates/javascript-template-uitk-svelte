<script lang="ts">
  import { BrandMark, ThemeToggle } from '$lib';
</script>

<svelte:head><title>Example brand kit</title></svelte:head>

<main>
  <header>
    <BrandMark />
    <ThemeToggle />
  </header>

  <h1>Example</h1>
  <p class="lede">Shared brand identity, design tokens, and reusable Svelte components.</p>

  <section aria-labelledby="tokens">
    <h2 id="tokens">Tokens</h2>
    <p>
      Colors, type, and spacing live in <code>src/lib/styles/tokens.css</code>. The values there are neutral
      placeholders — replace them with the brand's own.
    </p>
  </section>
</main>

<style>
  main {
    width: min(calc(100% - 3rem), 64rem);
    margin: 0 auto;
    padding: 3rem 0 6rem;
  }

  header {
    display: flex;
    align-items: center;
    justify-content: space-between;
    gap: 1.5rem;
    padding-bottom: 1.5rem;
    border-bottom: 1px solid var(--example-border);
  }

  h1 {
    margin: 2.5rem 0 0.5rem;
    font-size: 1.75rem;
    font-weight: 600;
  }

  h2 {
    margin: 0 0 0.5rem;
    font-size: 1rem;
    font-weight: 600;
  }

  .lede {
    margin: 0;
    max-width: 60ch;
    color: var(--example-fg-muted);
  }

  section {
    margin-top: 2.5rem;
    max-width: 60ch;
  }

  section p {
    margin: 0;
    color: var(--example-fg-muted);
  }

  code {
    font-family: var(--example-font-mono);
  }
</style>
